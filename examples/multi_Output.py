...
original_param_dist = {
    'n_estimators': randint(50, 300),  # Increased range for exploration
    'max_depth': [None, 3, 5, 10],  # Adjusted example values
    'min_samples_split': randint(2, 20),  # Adjusting split samples
    'min_samples_leaf': randint(1, 10),  # Adjusting leaf samples
    # 'max_features': ['auto', 'sqrt'],  # Feature subset sizes

    'bootstrap': [True, False],  # Bootstrap sampling
    # More shared hyperparameters and their distributions to explore
}
...
class MarketAnalyzer:
    ...
    def optimize_hyperparameters_random_feedback(self, x_cls_train, y_cls_train, x_reg_train, y_reg_train, win_count,
                                                 loss_count):
        # Define wider parameter distributions for both Classifier and Regressor
        global original_param_dist
        param_dist = dict(original_param_dist)

        def adjust_parameters_for_losses():
            global original_param_dist
            param_dist1 = dict(original_param_dist)

            # Adjust hyperparameters to mitigate risk after losses
            param_dist1['max_depth'] = [None, 1]  # Decrease complexity post-loss
            param_dist1['min_samples_split'] = randint(2, 3)  # Further reduce splits for caution
            param_dist1['min_samples_leaf'] = randint(1, 2)  # Fine-tune leaf samples post-loss
            # Narrow down the range for other parameters to minimize risk post-loss

            return param_dist1

        def revert_to_original_parameters():
            global original_param_dist

            param_dist2 = dict(original_param_dist)  # Revert to original parameters
            # Adjust hyperparameters to mitigate risk after losses
            param_dist2['max_depth'] = [None, 1, 2]  # Decrease complexity after losses
            param_dist2['min_samples_split'] = randint(2, 5)  # Reduce splits for caution
            param_dist2['min_samples_leaf'] = randint(1, 3)  # Decrease leaf samples for stability
            # Add more parameters to reduce model sensitivity or risk exposure after losses

            return param_dist2

        # Adjust hyperparameters based on observed win/loss patterns more aggressively
        if win_count < loss_count:
            if loss_count >= 1:
                param_dist = adjust_parameters_for_losses()

            else:
                param_dist = revert_to_original_parameters()

        else:
            param_dist['max_depth'] = [None, 2, 3]  # Decrease max_depth aggressively
            param_dist['min_samples_split'] = randint(2, 15)  # Adjust other hyperparameters aggressively
            param_dist['min_samples_leaf'] = randint(1, 8)  # Adjust leaf samples aggressively
            param_dist['max_leaf_nodes'] = [None, 5, 10, 15]  # Further exploration of leaf nodes
            # Add more hyperparameters to adjust based on observed patterns

        # RandomizedSearch for Classifier
        random_search_cls = RandomizedSearchCV(RandomForestClassifier(), param_dist, cv=5, n_iter=15)
        random_search_cls.fit(x_cls_train, y_cls_train)

        # RandomizedSearch for Regressor
        random_search_reg = RandomizedSearchCV(RandomForestRegressor(), param_dist, cv=5, n_iter=15)
        random_search_reg.fit(x_reg_train, y_reg_train)

        best_classifier = random_search_cls.best_estimator_
    ...
    def predict_and_trade(self, classifier, regressor, features):
        ...
        magnitude_prediction = classifier.predict(features)[0]

        # Predict trend direction
        trend_prediction = self.predict_trend(classifier, features)

        # Predict price change
        price_change_prediction = regressor.predict(features)[0]

        # Trading Strategy based on predictions
        if magnitude_prediction == 0 and trend_prediction == 0 and price_change_prediction > 0.5:
        ...
        if magnitude_prediction != 0 and trend_prediction == 4 and price_change_prediction < -0.5:
...
if __name__ == "__main__":
    ...
    test_size = 0.2
    if len(x_cls_train) < 5:  # Adjust the threshold based on your dataset size
        test_size = 0.1
        # Split data into training and test sets for both classification and regression

    x_cls_train, x_cls_test, y_cls_train, y_cls_test = train_test_split(x_cls_train, y_cls_train, test_size=test_size,
                                                                        random_state=42)
